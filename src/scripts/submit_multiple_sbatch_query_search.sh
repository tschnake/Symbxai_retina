...
for ids in $treebank_ids; do
  sbatch --mem=128G run_query_auto_search_apptainer_wrapper.sh --sample_range "[${ids}]" --comp_mode 'harsanyi' --max_and_order ${max_and_order} --datamode "${datamode}" --harsanyi_maxorder ${harsanyi_maxorder} --weight_modes "['occlusion']" --max_setsize ${max_setsize} --logfolder "${logfolder}" --resultfolder "${resultfolder}" &
