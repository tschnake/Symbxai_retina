...
def symb_xai(rels, feats, mode='subset', context=None):
    assert mode in ['subset', 'and', 'or', 'not'], f'Mode "{mode}" is not implemented.'
    r = 0.
    if context is not None:
        rels = copy.copy(rels)
        rels = {w:rel for w,rel in rels.item() if all([n in context for n in w])}
...
class HOExplainer:
    def __init__(self, model):
        ...
        self.modified_model = ModifiedTinyTransformerForSequenceClassification(
                                                                    model,
                                                                    order='first'
                                                                        )
